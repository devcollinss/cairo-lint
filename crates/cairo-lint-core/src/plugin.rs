...
pub fn diagnostic_kind_from_message(message: &str) -> CairoLintKind {
    match message {
        ...
        unwrap_used::UNWRAP_USED => CairoLintKind::UnwrapUsed,
            ...
            for node in function_nodes {
                match node.kind(syntax_db) {
                    SyntaxKind::ExprParenthesized =>
                        double_parens::check_double_parens(
                            db.upcast(),
                            &AstExpr::from_syntax_node(db.upcast(), node),
                            &mut diags
                        ),
                    SyntaxKind::StatementBreak =>
                        breaks::check_break(db.upcast(), node, &mut diags),
                    SyntaxKind::ExprIf => {
...
fn check_function(
    ...
    for (_expression_id, expression) in &function_body.arenas.exprs {
        match &expression {
            ...
            Expr::Loop(expr_loop) => {
                loops::check_loop_match_pop_front(
                    db,
                    expr_loop,
                    diagnostics,
                    &function_body.arenas
